...
即$$L(x_1,x_2,...,x_n;\hat{\theta}) = max L(x_1,x_2,...,x_n;\theta)$$，由此求得的最大值$$\theta$$可以用$$x_1,x_2,...,x_n$$表示，$$\hat{\theta}=\hat{\theta} (x_1,x_2,...,x_n)$$,从而统计量$$\hat{\theta}= \hat{\theta} ( X_1,X_2,...,X_n )$$，求解过程可用用拉格朗日乘子法。  例如：（1）X服从参数为$theta$的指数分布，则 $theta$的极大似然估计为$$\hat{\theta}=\frac{1}{\overline{x}}=\frac{n}{ \sum x_i }$$  
